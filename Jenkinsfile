        ...
        script {
          ...
          for (def ENV_NS : envs) {
            sh """
              ...
              # 🔥 FIXED: ENV-specific app names with proper error handling
        ...
        script {
          ...
          for (def ENV_NS : envs) {
            sh """
              ...
              kubectl get endpoints -n ${ENV_NS}  # 🔥 Check DB endpoints!
            """
          }
          sh """
            echo "=== ARGOCD APPS ==="
            kubectl get applications -n argocd | grep -E "(frontend|backend|database)-(dev|qa)" || echo "No matching apps found"
          """
        }
      }
    }
